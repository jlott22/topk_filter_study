...
# DGA allocator state
# -----------------------------
# Resource-bounded hardware DGA. The simulator defaults (30 population, 25
# generations, all grid cells) exceed practical RP2040 heap headroom, so this
# port preserves the genetic operators/objective while bounding the working set.
DGA_POPULATION_SIZE = 12
DGA_ITERATIONS_PER_TRIGGER = 8
DGA_COMMITMENT_HORIZON = 3
DGA_MAX_CANDIDATES = 48
...
def _dga_candidates():
    ...
    ranked.sort()
    cells = [item[2] for item in ranked]
    if DGA_MAX_CANDIDATES and len(cells) > DGA_MAX_CANDIDATES:
        cells = cells[:DGA_MAX_CANDIDATES]
    return cells
...
def _dga_edge_cost(previous, cell):
    ...
    return distance + 8.0 * (1.0 - target_p[idx(cell[0], cell[1])])
...
def _dga_crossover(parent_a, parent_b, team, candidates):
    ...
    for rid in sorted(team, key=_rid_sort_key):
        ...
        cut_a = len(a) // 2
        cut_b = len(b) // 2
        child[rid] = list(a[:cut_a]) + list(b[cut_b:])
...
def _dga_mutate(plan, team, candidates):
    ...
    if operation == 0:
        ...
        result[destination].insert(random.randrange(len(result[destination]) + 1), cell)
    elif operation == 1:
        source = random.choice(sources)
        route = result[source]
        if len(route) >= 2:
...
def _dga_run():
    global dga_population, dga_generation, dga_received_solution_pool
    candidates = _dga_candidates()
    team = _dga_team_agents()
    if not candidates or not team:
        _dga_commit({str(ROBOT_ID): []}, 1000000000000)
        dga_population = []
        return

    population = []
    for plan in dga_population + dga_received_solution_pool:
        population.append(_dga_repair_plan(plan, team, candidates))
    population.append(_dga_greedy_seed(team, candidates))
    if dga_path:
        seed = _dga_greedy_seed(team, candidates)
        own = [cell for cell in dga_path if cell in candidates]
        used = set(own)
        seed[str(ROBOT_ID)] = own + [cell for cell in seed.get(str(ROBOT_ID), []) if cell not in used]
        for rid in seed:
            if rid != str(ROBOT_ID):
                seed[rid] = [cell for cell in seed[rid] if cell not in used]
        population.append(_dga_repair_plan(seed, team, candidates))
    while len(population) < DGA_POPULATION_SIZE:
    ...
    for _ in range(DGA_ITERATIONS_PER_TRIGGER):
        ...
        next_population = [_dga_copy_plan(item[0]) for item in scored[:DGA_ELITE_COUNT]]
        while len(next_population) < DGA_POPULATION_SIZE:
            a = _dga_tournament(scored)
            b = _dga_tournament(scored)
            child = _dga_crossover(a, b, team, candidates) if random.random() < DGA_CROSSOVER_RATE else a
...
def _dga_reconstruct_received(sender, solution_id):
    ...
    for owner, entries in solution["owners"].items():
        size = solution["sizes"].get(owner, 0)
        route = []
...
def _dga_receive_payload(sender, payload):
    ...
    plan = _dga_reconstruct_received(str(sender), solution_id)
    if plan:
        ...
        local_scaled = int(dga_best_fitness * DGA_FITNESS_SCALE)
        if fitness_scaled < local_scaled or (
                fitness_scaled == local_scaled and generation > dga_generation):
...
def _dga_reset_if_new_clue_information():
    ...
    global dga_path, dga_pending_deltas, dga_received_solution_pool
...
def _reset_allocator_for_next_trial():
    ...
    global dga_received_solution_pool, dga_received_better_solution, dga_delta_counter
    dga_population = []
    dga_best_plan = {}
    dga_best_fitness = 1000000000000
    dga_path = []
