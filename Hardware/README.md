...
- `Algorithms/Pololu_*.py` contains temporary programs copied from the benchmark
...
- `../Simulation/Archive/Legacy/in_the_works/Pololu_*.py` contains the archived
...
- `Hubs/metrics_hub.py` remains the controller for both sets and now restricts
...
- `Algorithms/Pololu_*.py` - temporary known-bootable robot programs with a lightweight
  Top-K candidate prefilter.
- `../Simulation/Archive/Legacy/in_the_works/` - archived memory-heavy Top-K/parity robot
  programs.
...
- `ScenarioTools/clue_object_generator_manhat.py` - trial target/clue generator.
...
```bash
python Hardware/Hubs/metrics_hub.py --algorithm ACBBA --trials 1
```
...
`Results/Hardware/PilotTrials/hardware_handpicked_5_scenario_manifest.json`; a later algorithm or
...
`../Simulation/Architecture/simulator/scenarios/final_trial_500.csv`; the similarly named files under
