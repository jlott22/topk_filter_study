...
OUTPUT_DIR = REPO_ROOT / "results" / "unified_topk_study"
LOWK_DIR = (
    REPO_ROOT
    / "results"
    / "hil_aligned_simulation"
    / "missing_lowk_full_counts"
    / "combined"
)
...
def build() -> None:
    missions = {
        "bayesian": (*load_bayesian_rows(), "bayesian_topk_unified.csv"),
        "collaborative": (*load_collaborative_rows(), "collaborative_topk_unified.csv"),
    }
    summaries: dict[str, object] = {}
    for mission, (rows, expectations, filename) in missions.items():
        sort_rows(rows)
